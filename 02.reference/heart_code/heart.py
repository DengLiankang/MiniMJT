...
fixed_scale_range = (3.9, 4.2)
min_scale = np.array([1.0, 1.0, 1.0]) * 0.9
max_scale = np.array([1.0, 1.0, 1.0])

# 外围随机心参数
random_point_size = 5000
random_scale_range = (3.5, 3.9)
random_point_maxvar = 0.1
...
# 随机点文件缓存
points_file = output_dir + "/points.txt"
...
def paint_heart(ratio, randratio, outputFile=None):
    ...
    for i in range(fixed_point_size):
        ...
        dist = distance(point)
        radius = 1.0
        sphere_scale = radius * 0.5 / dist
        point = point * lerp_float(0.9, sphere_scale, ratio * 0.1)
    ...
    randPoints = genRandPoints(
        random_point_size, random_scale_range, random_point_maxvar, randratio)
