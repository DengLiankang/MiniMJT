...
static void screen_share_process(AppController *sys, const ImuAction *action)
{
    ...
    if (0 == run_data->tcp_start && 0 == run_data->req_sent) {
        // 预显示
        display_screen_share("Screen Share", WiFi.softAPIP().toString().c_str(), "8081", "Wait connect ....",
        ...
        // 使用STA模式
        sys->send_to(SCREEN_SHARE_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_CONN, NULL, NULL);
        ...
        if (doDelayMillisTime(SHARE_WIFI_ALIVE, &run_data->pre_wifi_alive_millis, false)) {
            // 发送wifi维持的心跳
            sys->send_to(SCREEN_SHARE_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_ALIVE, NULL, NULL);
        }
        ...
        if (ss_client.connected()) {
            ...
            if (ss_client.available()) {
                ...
                int32_t read_count =
                    ss_client.read(&run_data->recvBuf[run_data->bufSaveTail], 10000); // 向缓冲区读取数据
            ...
            // 预显示
            display_screen_share("Screen Share", WiFi.localIP().toString().c_str(), "8081", "Wait connect ....",
                                 LV_SCR_LOAD_ANIM_NONE);
...
static void screen_message_handle(const char *from, const char *to, APP_MESSAGE_TYPE type, void *message,
                                  void *ext_info)
{
    switch (type) {
        case APP_MESSAGE_WIFI_CONN: {
            Serial.print(F("APP_MESSAGE_WIFI_CONN enable\n"));
            display_screen_share("Screen Share", WiFi.localIP().toString().c_str(), "8081", "Connect succ",
                                 LV_SCR_LOAD_ANIM_NONE);
        ...
        case APP_MESSAGE_WIFI_AP: {
            ...
            display_screen_share("Screen Share", WiFi.softAPIP().toString().c_str(), "8081", "Connect succ",
                                 LV_SCR_LOAD_ANIM_NONE);
            run_data->tcp_start = 1;
            // ss_server.begin(HTTP_PORT); //服务器启动监听端口号
            // ss_server.setNoDelay(true);
