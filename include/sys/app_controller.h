...
public:
    ...
    void RequestProcess(APP_MESSAGE_TYPE type, const char *key, char *value);

    // wifi事件的处理
    bool wifi_event(APP_MESSAGE_TYPE type);

    int16_t ReadFile(enum RW_FILE_SRC type, const char *path, void *cfgData);
    int8_t WriteFile(enum RW_FILE_SRC type, const char *path, void *cfgData);
...
private:
    ...
    void WifiConnectRequestDeal(void);
...
private:
    ...
    std::vector<EVENT_OBJ> eventList;   // 用来储存事件
